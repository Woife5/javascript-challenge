...
const possibleWays = [];
function findAllWays(cave, currentPos = '', visited = new Set(), twice = true) {
    if (cave.name === 'end') {
    ...
    for (let con of cave.connections) {
        if (con.name === 'start') continue;
        if (!con.isUpper && twice && visited.has(con.name)) {
            findAllWays(con, `${currentPos}${cave.name},`, new Set(visited), false);
        }
        if (!visited.has(con.name) || con.isUpper) {
            findAllWays(con, `${currentPos}${cave.name},`, new Set(visited), twice);
        }
    }
}
