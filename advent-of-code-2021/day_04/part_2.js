...
draws.forEach(draw => {
    boards.forEach(board => {
        if (board.hitNumber(parseInt(draw))) {
            lastWinningBoardSum = board.sumAllUnmarked();
            lastWinningNumber = parseInt(draw);
        }
    });
});

console.log(`Tha last winning board had a score of: ${lastWinningBoardSum * lastWinningNumber}`);
